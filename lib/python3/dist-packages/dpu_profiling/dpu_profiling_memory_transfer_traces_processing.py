...
def probe_libdpu__libdpu_dpu_transfer_matrix_add_dpu(
    ...
    mram_xfer_size.setdefault(transfer_matrix, {})[dpu] = True
...
def probe_libdpu__libdpu_dpu_copy_wram_for_matrix_add_dpu(
        event_name, context, common_cpu, common_secs, common_nsecs,
        common_pid, common_comm, common_callchain,
        __probe_ip,
        perf_sample_dict):
    global wram_xfer_size
    wram_xfer_size.setdefault(common_pid, []).append(True)


def probe_libdpu__libdpu_dpu_copy_wram_for_matrix_clear_all(
        event_name, context, common_cpu, common_secs, common_nsecs,
        common_pid, common_comm, common_callchain,
        __probe_ip,
        perf_sample_dict):
    global wram_xfer_size
    if common_pid in wram_xfer_size:
        del(wram_xfer_size[common_pid])
...
def probe_libdpu__libdpu_dpu_copy_to_wram_for_matrix_exit__return(
    ...
    end_ns = Duration.nanoseconds(common_secs, common_nsecs)
    path = rank_path[wram_pid[common_pid][1]]
    nr_dpus = len(wram_xfer_size[common_pid]
                  ) if common_pid in wram_xfer_size else 64
